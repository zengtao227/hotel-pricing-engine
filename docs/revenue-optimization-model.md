# 收益最大化定价模型

本文档解释系统为什么不是“预测一个房价”，而是用收益管理模型计算更可能带来最大收益的推荐价。

当前代码实现的是 **v1 可解释候选价收益模拟模型**。它不是黑盒机器学习模型，而是一个透明、可审计、可逐步升级的数学优化框架：

```text
需求预测 -> 价格弹性 -> 候选价收益模拟 -> 约束下选择预期收益最高价格
```

## 1. 给酒店管理者的直观解释

酒店收益管理的核心不是“把房间全部卖完”，也不是“把价格定得越高越好”，而是在 **房价** 和 **销量 / 出租率** 之间找到一个收益最高的组合。

最简单的收入公式是：

```text
Revenue = Price × Sold Rooms
```

也就是说，收益同时取决于两个因素：

- 每间房卖多少钱
- 最后能卖出多少间房

所以，100% 入住率并不一定代表最高收益。

例如一家酒店有 100 间房：

| 房价 | 出租率 | 卖出房间 | 总收入 |
|---:|---:|---:|---:|
| 300 | 100% | 100 | 30,000 |
| 400 | 90% | 90 | 36,000 |
| 500 | 75% | 75 | 37,500 |
| 650 | 50% | 50 | 32,500 |

在这个例子里，最高收入不是 100% 入住，而是：

```text
500 × 75 = 37,500
```

这说明酒店定价不能只看出租率。价格太低时，房间可能很容易卖满，但每间房收入太低；价格太高时，单间收入高，但卖出的房间太少。真正的收益最大点通常在中间：价格足够高，销量也足够高。

因此，系统真正要回答的问题不是：

```text
怎样把房间全部卖完？
```

而是：

```text
剩下这些房间，在未来销售窗口里，以什么价格卖，预期总收益最高？
```

这也是为什么系统会同时看：

- 当前已经卖出多少房
- 还剩多少可售库存
- 离入住日期还有多久
- 最近 7 天 / 14 天新增了多少订单
- 历史类似日期通常能卖多少
- 当前价和推荐价分别对应多少预期收益

如果离入住日还远，但已经卖出很多房、近期 pickup 很强，说明需求可能偏强，系统可能建议涨价。因为剩余库存变少，没有必要太便宜卖。

如果快到入住日了，但库存还很多、近期 pickup 很弱，说明需求可能不足，系统可能建议降价。因为房间是易逝库存，入住日过去后，没卖掉的房间价值就变成 0。

如果当前价已经接近收益最优点，系统会建议保持。因为频繁小幅调价会增加运营复杂度，但收益提升可能很有限。

所以更准确的管理者理解是：

```text
收益不是由出租率单独决定，也不是由房价单独决定，而是由房价和销量共同决定。
系统要找的是在给定库存和需求条件下，让“价格 × 预计销量”最大的价格点。
```

进一步说，已经卖出的房间收入已经锁定，今天改价不会改变这些订单。真正需要优化的是剩余库存未来还能以什么价格卖出去。

## 2. 剩余库存的价格是怎么来的

假设某一天某个房型总共有 100 间，现在已经卖出 70 间，还剩 30 间。系统并不是重新给全部 100 间房定价，而是把问题缩小到：

```text
已经卖出的 70 间收入已经锁定；
剩下 30 间，在未来销售窗口里应该用什么价格卖，预期总收益最高？
```

模型会先估计：如果当前价格不变，最终大概能卖到多少间。

例如当前挂牌价是 388，系统会看历史类似日期、当前销售进度、近期 pickup、剩余库存和离入住日还有多少天。假设模型判断：

```text
如果继续卖 388，最终可能卖到 90 间。
```

这意味着：

```text
已经卖出：70 间
未来预计还能再卖：20 间
剩余库存：30 间
```

接下来，系统会生成一组真实可执行的候选价格，例如：

```text
368 / 388 / 408 / 428 / 448 / 468 / 488
```

如果使用中国酒店演示的价格尾数规则，也可能是：

```text
388 / 428 / 468 / 488 / 508
```

然后系统会逐个计算：

| 候选价 | 未来预计还能卖出 | 未来新增收入 | 总预期收入 |
|---:|---:|---:|---:|
| 388 | 20 间 | 7,760 | 34,920 |
| 428 | 19 间 | 8,132 | 35,292 |
| 468 | 16 间 | 7,488 | 34,648 |
| 488 | 14 间 | 6,832 | 33,992 |

在这个简化例子里，系统会推荐 428，因为虽然预计少卖 1 间，但单间价格提高后，总预期收益更高。

所以“剩下 30 间房到底卖多少钱”不是凭感觉来的，而是来自以下步骤：

```text
1. 看历史类似日期，估计正常最终需求
2. 看当前已经卖出多少，判断当前销售进度
3. 看近期 pickup，判断最近需求强弱
4. 看剩余库存，判断库存压力
5. 用价格弹性估计不同价格下未来还能卖多少
6. 生成一组可执行候选价格
7. 分别计算每个候选价的预期收益
8. 选择预期收益最高的价格
```

旧数据的作用不是直接告诉系统“今天应该卖多少钱”，而是帮助模型回答这些问题：

- 这个房型在类似日期通常卖得快不快？
- 这个价格区间以前是否容易成交？
- 周末和工作日需求差多少？
- 提前 30 天、提前 7 天、临近入住时，订单增长速度通常怎样？
- 当前销售进度比历史正常水平快还是慢？

换句话说，我们确实是在用一个数学模型，把旧价格、旧出租率、历史销量、当前已售、剩余库存、近期订单等参数放进去，然后模型计算不同候选价下的预期收益，最后选择预期收益最高的价格。

但要注意：系统选择的是 **当前数据和模型假设下的预期收益最高价**，不是保证真实世界一定最高的价格。真实世界还会受到竞品调价、活动取消、天气变化、OTA 排名、临时团队订单等因素影响。

## 3. 业务问题的数学形式

酒店房间是典型的易逝库存：某个入住日期过去后，没卖出去的间夜不能再售卖。因此每个 `入住日期 x 房型` 都可以看成一个有限库存收益最大化问题。

对某个入住日期和房型，定义：

| 符号 | 含义 |
|---|---|
| `p0` | 当前价 |
| `p` | 候选推荐价 |
| `N` | 可售库存 |
| `S0` | 观察日已经售出的间夜 |
| `R0` | 观察日已经锁定的房费收入 |
| `C = max(N - S0, 0)` | 剩余可售库存 |
| `D0` | 当前价不变时的最终需求预测 |
| `epsilon` | 价格弹性，通常为负数 |

系统的目标不是最大化房价，也不是最大化入住率，而是最大化预期房费收入：

```text
p* = argmax ExpectedRevenue(p)
```

## 4. 已知订单和未来需求分开处理

已经成交的订单不应该因为今天改价而消失，所以模型把收入拆成两部分：

```text
总预期收益 = 已锁定订单收入 + 未来新增订单预期收入
```

也就是：

```text
ExpectedRevenue(p) = R0 + p * ExpectedNewSoldRooms(p)
```

其中 `R0` 是观察日已经知道的订单收入，只有未来尚未成交的需求会受到候选价 `p` 影响。这个设计可以避免一个常见错误：把已经售出的房间也当成会随价格变化而重新成交。

## 5. 当前价下的需求预测

v1 用两个信号估计当前价不变时的最终需求 `D0`：

1. 历史类似日期的基准入住率
2. 当前预订进度和最近 14 天 `Pickup`

公式如下：

```text
HistoricalDemand = baseline_occupancy * N
PaceProjection = S0 + pickup_14d * lead_time_factor
D0 = clip(
    w * PaceProjection + (1 - w) * HistoricalDemand,
    lower = S0,
    upper = N
)
```

其中：

- 离入住日期越近，`w` 越高，因为当前预订进度更可信。
- 离入住日期越远，历史基准占比更高，因为未来还有较长销售窗口。
- `clip` 保证预测需求不会低于已售间夜，也不会超过可售库存。

## 6. 价格弹性模型

价格弹性描述“价格变化 1% 时，需求大约变化多少百分比”。经济学中常用定义是：

```text
price_elasticity = % change in quantity demanded / % change in price
```

酒店定价里弹性通常为负数。例如 `epsilon = -1.2` 表示价格提高 1%，未来未成交需求大约下降 1.2%。

v1 使用常数弹性需求函数：

```text
FutureDemand(p) = max(D0 - S0, 0) * (p / p0) ^ epsilon
```

最终预计售出间夜为：

```text
ExpectedSoldRooms(p) = S0 + min(C, FutureDemand(p))
```

于是候选价 `p` 的预期收益为：

```text
ExpectedRevenue(p) = R0 + p * min(C, max(D0 - S0, 0) * (p / p0) ^ epsilon)
```

### 弹性如何估计

当前阶段没有足够真实实验数据，所以系统先采用透明规则估计弹性：

- 默认弹性：`-1.25`
- 周末、入住率高、剩余库存少、近期 pickup 强时，需求更刚性，弹性绝对值降低
- 入住率低、临近入住但剩余库存高、近期 pickup 弱时，需求更敏感，弹性绝对值提高
- 最终限制在 `[-2.20, -0.60]`，避免模型给出极端反应

这不是最终的机器学习模型，而是可解释的 v1 基线。它的价值是：每一个推荐价都能说明“为什么涨价或降价会影响收益”。

## 7. 价格策略：挂牌价、折扣价和渠道价

当前系统里的 `current_price` 和 `recommended_price` 默认表示 **酒店内部准备管理和审批的挂牌价 / 标准可售价格**。这适合做收益管理决策，因为酒店需要先确定一个统一的价格基准，再决定不同渠道如何展示。

但是在真实销售中，Booking.com、携程、飞猪、美团、官网会员价、企业协议价等渠道可能会有不同折扣或佣金结构。因此，最终对客展示价可能不是系统里的挂牌价，而是由挂牌价经过渠道策略转换得到。

常见价格口径包括：

| 价格口径 | 含义 | 示例 |
|---|---|---|
| 挂牌价 / 标准价 | 酒店内部审批和管理的基础价格 | 468 |
| OTA 展示价 | Booking、携程等平台对客显示的价格，可能含促销或会员折扣 | 438 |
| 渠道净价 | 扣除佣金或渠道成本后，酒店实际到手收入 | 438 × (1 - 佣金率) |
| 企业协议价 | 面向公司客户的合同价 | 398 |
| 官网会员价 | 面向会员或直销渠道的折扣价 | 448 |

因此，真实产品中可以把价格分为两层：

```text
收益管理推荐价 = 酒店内部批准的基准挂牌价
渠道展示价 = 基准挂牌价 × 渠道折扣 / 促销规则
渠道净收益 = 渠道展示价 - 佣金 / 成本
```

第一阶段建议仍然以挂牌价作为模型推荐对象，因为它最容易被酒店管理者理解，也最适合做审批流程。后续接入 Channel Manager 或 OTA 时，可以增加一个“渠道价格策略层”：

```text
Approved Base Price
        ↓
Channel Pricing Rules
        ↓
Booking.com Price / Ctrip Price / Direct Website Price / Corporate Rate
        ↓
Net Revenue After Commission
```

这样做的好处是：

- 收益经理只需要审核一个清晰的基准价格。
- 不同渠道可以根据合同、佣金和促销策略自动生成展示价。
- 模型未来可以从“最大化房费收入”升级为“最大化扣除佣金后的净收益”。

例如，系统批准某房型基准价为 468：

| 渠道 | 规则 | 展示价 | 酒店净收入示意 |
|---|---|---:|---:|
| 官网直销 | 无折扣 | 468 | 468 |
| 官网会员 | 95 折 | 445 | 445 |
| 携程促销 | 92 折 | 431 | 431 - 渠道成本 |
| Booking.com | 平台折扣或 Genius 价 | 例如 438 | 438 - 佣金 |
| 企业协议 | 固定协议价 | 398 | 398 |

所以，在演示中可以这样解释：

```text
当前系统推荐的是酒店内部审批用的基准挂牌价。
真正发布到 Booking、携程或官网时，还可以叠加渠道折扣、会员价、促销价和佣金规则。
后续版本会把这部分做成 Channel Pricing Rules，让同一个批准价自动生成不同渠道的可发布价格。
```

## 8. 候选价集合和约束优化

真实酒店价格不是任意连续数字，必须满足业务约束：

- 单次最大调价幅度
- 房型最低价和最高价
- 市场友好的尾数规则，例如中国酒店常用 6/8/9 尾数
- 人工审批前不自动写价
- 小幅收益提升不足以覆盖运营摩擦时保持当前价

所以系统不直接求连续闭式解，而是生成一组可执行候选价：

```text
CandidatePrices = rounded prices within:
[
    p0 * (1 - max_change_pct),
    p0 * (1 + max_change_pct)
]
and price_floor <= p <= price_ceiling
```

然后枚举每个候选价：

```text
for p in CandidatePrices:
    revenue[p] = ExpectedRevenue(p)

p* = price with max revenue[p]
```

如果最佳候选价相对当前价的收益提升低于 `0.5%`，系统会建议 `hold`，避免频繁微调价格。

这种离散优化方式更适合早期产品，因为它和酒店真实操作一致：收益经理最终关心的是“今天可不可以从 388 调到 428”，而不是一个无法上架的理论价格 `421.37`。

## 9. 推荐输出如何解释

每条推荐现在会输出：

- `current_expected_revenue`: 当前价下的预期收益
- `recommended_expected_revenue`: 推荐价下的预期收益
- `expected_revenue_delta`: 两者差值
- `demand_forecast_at_current_price`: 当前价不变时的最终需求预测
- `expected_sold_rooms`: 推荐价下预计最终售出间夜
- `expected_new_sold_rooms`: 推荐价下预计新增成交间夜
- `demand_elasticity`: 本条建议使用的价格弹性
- `candidate_price_count`: 实际比较过的候选价数量

客户可以用这些字段追问模型：

```text
为什么建议涨价？
因为当前 pickup 强、剩余库存低，模型判断需求更刚性。
在当前价 388 下，预期收益是 X；
在候选价 428 下，虽然预计成交间夜略低，但单间收入提升后，总预期收益更高。
```

这比只说“系统建议上调 10%”更可信，也更容易被收益经理复核。

## 10. 当前回测如何使用收益模拟

当前 Backtest 页面已经升级为双口径：

1. 价格弹性收益回测：使用历史观察日以前已知订单生成推荐，并汇总当时模型估计的当前价预期收益、推荐价预期收益和预期收益变化。
2. 静态实际销量对照：用最终真实售出间夜做保守 sanity check，观察如果销量不变，推荐价相对当前价的收入差。

静态对照口径为：

```text
基准收入 = 当前价 * 最终实际售出房间数
推荐价静态收入 = 推荐价 * 最终实际售出房间数
```

它适合检查推荐方向和数据泄漏问题，但它假设销量不随价格变化，所以不能严格证明收益最大化。

价格弹性收益回测使用的是动态收益模拟：

```text
推荐价改变 -> 未来需求改变 -> 预计售出间夜改变 -> 预期收益改变
```

页面还会展示一条样本的候选价收益曲线，帮助客户看到“为什么推荐价是这一组候选价中的收益最高点”。这仍然是模型预期值，不是已经证明的真实因果收益；真实客户数据积累后，应继续做更严格的 holdout 回测、审批后表现追踪和实验验证。

## 11. 生产级模型升级路线

v1 模型适合演示和早期人工复核。真实客户数据积累后，可以升级为更强的统计模型。

### 11.1 需求到达模型

可以把未来新增预订视为随机变量：

```text
Y(p) ~ Poisson(lambda(p))
```

其中：

```text
lambda(p) = exp(beta0 + beta_x * features + beta_p * log(p))
```

`features` 可以包括星期、月份、节假日、提前期、渠道、房型、竞品价格、活动日和历史 pickup。

### 11.2 成交概率模型

如果有曝光、询价或搜索数据，可以建成交概率：

```text
Pr(book | p, x) = sigmoid(beta0 + beta_x * x + beta_p * log(p))
```

再计算：

```text
ExpectedBookings(p) = traffic_forecast * Pr(book | p, x)
```

### 11.3 有限库存截断

因为最多只能卖 `C` 间夜，所以生产模型应计算：

```text
ExpectedNewSoldRooms(p) = E[min(Y(p), C)]
```

这比简单 `min(E[Y], C)` 更严谨，尤其适合高需求日期。

### 11.4 分层价格弹性

当真实数据足够后，弹性不应只有一个全局值，而应按层级估计：

```text
epsilon = global + room_type_effect + day_of_week_effect + season_effect + lead_time_effect + channel_effect
```

这样可以让标准间、套房、周末、淡季、临近入住等场景拥有不同的价格敏感度。

### 11.5 渠道价格和净收益优化

当系统接入真实渠道数据后，模型可以进一步从“房费收入最大化”升级为“渠道净收益最大化”。这需要引入：

- 渠道佣金率
- OTA 促销折扣
- 官网会员价规则
- 企业协议价
- 不同渠道的转化率和流量
- 渠道配额和库存控制

生产级优化目标可以变成：

```text
ExpectedNetRevenue(channel, p) = DisplayPrice(channel, p) × ExpectedSoldRooms(channel, p) - ChannelCost(channel)
```

这样系统不只是回答“这个房型今天卖多少钱”，还可以回答：

```text
官网、携程、Booking、企业协议价各应该发布什么价格，才能让酒店净收益更高？
```

## 12. 科学依据

本方案参考了收益管理和价格弹性两个成熟方向：

- Gallego and van Ryzin, 1994, [*Optimal Dynamic Pricing of Inventories with Stochastic Demand over Finite Horizons*](https://pubsonline.informs.org/doi/10.1287/mnsc.40.8.999). 该论文讨论了有限库存、有限销售周期、随机且价格敏感需求下的动态定价问题，并明确提到酒店房间这类到期后失去价值的库存。
- OpenStax, *Principles of Economics 3e*, [“Price Elasticity of Demand and Price Elasticity of Supply”](https://openstax.org/books/principles-economics-3e/pages/5-1-price-elasticity-of-demand-and-price-elasticity-of-supply). 该教材给出价格弹性的标准定义，即需求量变化百分比与价格变化百分比之比。

v1 的重点不是宣称已经得到完美最优解，而是把推荐价从“经验规则”提升为“可解释的约束收益优化”。这为后续接入真实 PMS / Channel Manager 数据、学习真实价格弹性、做收益回测和自动化审批打下基础。

## 13. 当前实现文件

- `src/revenue_simulation.py`: 候选价收益模拟和价格弹性估计
- `src/pricing_engine.py`: 调用收益模拟并生成推荐价、解释、风险提示和收益字段
- `src/i18n.py`: 推荐结果字段的多语言标签
- `src/ui_help.py`: 页面中的指标解释和字段提示
