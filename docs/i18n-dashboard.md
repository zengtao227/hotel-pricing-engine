...
3. **回测分析**
   ...
   - 使用价格弹性模型做预期收益回测，并保留最终实际售出房间数的静态对照
   - 展示当前价预期收益、推荐价预期收益、弹性预期收益变化和静态销量对照
   - 展示每日弹性/静态收益变化对比图和候选价收益曲线
   - 支持导出回测明细 Excel
...
销售看板和调价建议页都包含“如何解读这些指标”的折叠说明。重点解释：
...
- `src/backtesting.py`：负责价格弹性收益回测、候选价收益曲线、静态销量对照和回测明细导出。
...
- `app/streamlit_app.py`：负责页面布局、语言选择、销售看板、表格展示、导出语言选择和 tooltip 接入。
